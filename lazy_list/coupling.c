...
int lockc_find(intset_l_t *set, val_t key, val_t *val) {
	node_l_t *curr, *next;
	int found;
...
int lockc_insert(intset_l_t *set, val_t key, val_t val) {
	...
	found = (key == next->key);
	if (!found) {
		newnode =  new_node_l(key, val, next);
		curr->next = newnode;
	}
  if(!found) count_success++;
  count++;
	UNLOCK(&curr->lock);
	UNLOCK(&next->lock);
	return !found;
