...
#ifdef DEBUG
extern int_aligned32_t __failed_cas[N_THREADS];
extern int_aligned32_t __executed_cas[N_THREADS];
extern int_aligned32_t __executed_swap[N_THREADS];
extern int_aligned32_t __executed_faa[N_THREADS];
#endif
...
inline  bool _CAS32(uint32_t *A, uint32_t B, uint32_t C) {
    ...
    res = __CAS32(A, B, C);
    __executed_cas[getThreadId()].v++;
    __failed_cas[getThreadId()].v += 1 - res;
